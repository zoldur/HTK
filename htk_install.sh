...
TMP_FOLDER=$(mktemp -d)
CONFIG_FILE='htk.conf'
CONFIGFOLDER='/root/.htk'
COIN_DAEMON='/usr/local/bin/htkd'
COIN_CLI='/usr/local/bin/htk-cli'
COIN_REPO='https://github.com/zoldur/HTK/releases/download/v3.0.0.0/htk.tar.gz'
COIN_NAME='HTK'
COIN_PORT=58100
RPCPORT=58200
...
function compile_node() {
  ...
  wget -q $COIN_REPO 
  compile_error 
  tar xvzf htk.tar.gz -C /usr/local/bin >/dev/null 2>&1
  compile_error 
  cd -
...
libminiupnpc-dev libgmp3-dev ufw fail2ban pkg-config libevent-dev unzip >/dev/null 2>&1
if [ "$?" -gt "0" ];
  then
